...
from pydantic_ai import NativeOutput, ModelSettings, Agent
...
class TitleClassifier:
    ...
    async def tiebreak_titles(self, titles, lines):
        ...
        confidence_results: List[TitleCandidateLine] = (
            await self.tiebreaker_agent.run(title_candidates)
        ).output
    ...
    async def classify_lines(self, lines):
        ...
        """
        indexed_lines = [
            json.dumps({"id": i, "text": line["text"], "geometry": line["geometry"]})
            for i, line in enumerate(lines)
        ]
        lookup = {i: p["text"] for i, p in enumerate(lines)}

        processed_lines: List[InformationLine] = (
            await self.classifier_agent.run(indexed_lines)
        ).output
        ...
        if len(titles) > 1:
            ...
            titles.sort(
                key=lambda x: (
                    confidence_map[x.id] if x.id and x.id in confidence_map else -1
                ),
                reverse=True,
            )
            ...
            title_translations = [
                ...
                if p.label == InformationLabel.TITLE_TRANSLATION
            ]
        ...
        classified_lines: List[EnrichedInformationLine] = [
            EnrichedInformationLine(
                text=lookup[line_info.id]
                if line_info.id is not None and len(lookup) > line_info.id > -1
                else "Desconocido",
        ...
        title = (
            lookup[titles[0].id]
            if titles[0].id is not None and len(lookup) > titles[0].id > -1
            else "Desconocido"
        )
        return title, classified_lines

    """ REMOVE THIS """
    async def obtain_titles(self, lines):
        indexed_lines = [
            json.dumps({"id": i, "text": line["text"], "geometry": line["geometry"]})
            for i, line in enumerate(lines)
        ]
        ...
        processed_lines: List[TitleCandidateLine] = (
            await self.tiebreaker_agent.run(indexed_lines)
        ).output

        processed_lines.sort(key=lambda x: x.confidence)

        titles: List[EnrichedTitleCandidateLine] = [
            EnrichedTitleCandidateLine(
                text=lookup[line_info.id]
                if line_info.id and len(lookup) > line_info.id > -1
                else "Desconocido",
